    ...
    <div className="pointer-events-none w-screen h-svh fixed top-0 left-0 bg-black mix-blend-difference z-[99999]">
      <div
        ...
        style={{
          ...
          display: outOfScreen ? 'none' : 'inline-block'
        }}
        id="cursor"
        className="cursor pointer-events-none w-10 h-10 mix-blend-difference rounded-100 relative z-50 invert pc:inline-blockhidden pc:flex items-center justify-center hidden"
      >
        <div
          style={{ transform: `${hovered ? 'scale(4)' : ''}` }}
          className="curso__inner w-full h-full rounded-100 invert opacity-80 bg-back absolute top-0 left-0 border bg-white ease-in-out duration-500 text-sm"
        ></div>
      </div>
    </div>
  );
